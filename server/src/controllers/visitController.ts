...
import { isMongoConnected, MOCK_VISITS } from '../data/mockFallback.js';
  ...
  try {
    if (!isMongoConnected()) {
      ...
      const results = patientId
        ? MOCK_VISITS.filter(v => v.patientId === patientId)
        : MOCK_VISITS;
...
export async function getVisitById(req: Request, res: Response) {
  try {
    ...
    if (!isMongoConnected()) {
      const visit = MOCK_VISITS.find(v => v._id === id);
      if (!visit) return res.status(404).json({ error: 'Visit encounter not found' });
...
export async function createVisit(req: Request, res: Response) {
  try {
    const {
      patientId, patientName, doctorName, doctorSpecialty, clinicOrHospital,
      date, visitType, chiefComplaint, symptoms, vitals, clinicalExamination,
      diagnosis, icd10Code, clinicalNotes, prescriptions, orderedLabTests, followUpDate,
    } = req.body;

    if (!patientId || !doctorName || !chiefComplaint || !diagnosis) {
      return res.status(400).json({ error: 'Missing required visit fields: patientId, doctorName, chiefComplaint, diagnosis' });
    }

    if (!isMongoConnected()) {
      const mockVisit = {
        _id: `visit-mock-${Date.now()}`,
        ...
        chiefComplaint, symptoms: symptoms || [], vitals: vitals || {},
        clinicalExamination, diagnosis, icd10Code, clinicalNotes,
        prescriptions: prescriptions || [], orderedLabTests: orderedLabTests || [],
    ...
    await PatientModel.findByIdAndUpdate(patientId, { lastVisitDate: saved.date });
